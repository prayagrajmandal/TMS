...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    if (selectedUser) {
      setEditableUser({
        ...
        role: selectedUser.roles[0] ?? "maintenance",
      })
    }
  }, [selectedUser])
  ...
  const saveSelectedUser = () => {
    ...
    updateSelectedUser((user) => ({
      ...
      department: editableUser.department.trim(),
      roles: [editableUser.role],
      accessRoutes: getDefaultAccessForRoles([editableUser.role]),
    }))
  }
          ...
          <div className="grid gap-5 lg:grid-cols-[320px_1fr]">
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-border bg-muted/30 p-4">
                ...
                <div className="grid gap-3 text-sm">
                  ...
                  <div className="rounded-lg bg-background p-3">
                    ...
                    <select
                      ...
                      onChange={(e) => setEditableUser({ ...editableUser, role: e.target.value as UserRole })}
                      className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-card-foreground"
                    >
                      {assignableRoles.map((role) => (
                        <option key={role} value={role}>
                ...
                <Button onClick={saveSelectedUser}>
                  Save User Changes
                </Button>
                <Button
                  variant="destructive"
                  onClick={() => {
                    ...
                    if (window.confirm(`Delete user ${selectedUser.userId}?`)) {
                      void deleteUser(selectedUser.organization, selectedUser.userId)
                      setSelectedUserId("")
                    }
                  }}
                >
